...
# ---------------------------------------------------------------------------
# v4.1.0 audit P1: the numeric Jacobian only recomputes the peaks a
# parameter can actually move.
# ---------------------------------------------------------------------------


def test_parameter_influence_maps_each_name_to_the_terms_it_moves():
    from peak_fit import _parameter_influence

    free = ["amp_0", "pos_0", "amp_1", "pos_1", "sigma", "tail_fraction",
            "tail_beta", "bg_c0", "bg_c1"]
    influence = _parameter_influence(free, n_peaks=2, link_widths=True)
    assert influence[0] == ([0], False)      # amp_0 -> peak 0 only
    assert influence[1] == ([0], False)      # pos_0 -> peak 0 only
    assert influence[2] == ([1], False)
    assert influence[3] == ([1], False)
    assert influence[4] is None              # linked sigma -> every peak
    assert influence[5] is None              # tail parameters -> every peak
    assert influence[6] is None
    assert influence[7] == ([], True)        # background only
    assert influence[8] == ([], True)


def test_parameter_influence_unlinked_sigma_is_per_peak():
    from peak_fit import _parameter_influence

    influence = _parameter_influence(
        ["amp_0", "pos_0", "sigma_0", "amp_1", "pos_1", "sigma_1"],
        n_peaks=2, link_widths=False,
    )
    assert influence[2] == ([0], False)
    assert influence[5] == ([1], False)


def test_parameter_influence_refuses_an_unknown_name():
    """Mirrors _build_param_damping's own refusal: a silently-wrong entry
    here would zero out part of a Jacobian column."""
    from peak_fit import _parameter_influence

    with pytest.raises(FitError, match="No influence rule"):
        _parameter_influence(["amp_0", "mystery"], n_peaks=1, link_widths=True)


def test_restricted_jacobian_equals_the_full_one():
    """The restricted evaluation is the SAME derivative: every term it
    leaves out is identical either side of the central difference and
    cancels exactly."""
    from peak_fit import (
        _build_param_damping, _make_model, _numeric_jacobian,
        _parameter_influence,
    )

    names = ["amp_0", "pos_0", "amp_1", "pos_1", "sigma"]
    model_named = _make_model(
        names, names, {}, n_peaks=2, link_widths=True, enable_left_tail=False
    )
    model = lambda xx, pp: model_named(xx, *pp)

    def restrict(peak_indices, include_background):
        def evaluate(xx, pp):
            values = dict(zip(names, pp))
            return model_named.evaluate(xx, values, peak_indices, include_background)
        return evaluate

    restricted = [
        None if scope is None else restrict(*scope)
        for scope in _parameter_influence(names, 2, True)
    ]

    x = np.arange(60, 140, dtype=float)
    p = np.array([1000.0, 95.0, 700.0, 115.0, 4.0])
    full = _numeric_jacobian(model, x, p)
    fast = _numeric_jacobian(model, x, p, restricted)
    # Not bit-identical, and the restricted one is the more accurate of the
    # two -- the full version subtracts large unchanged terms from
    # themselves and loses precision doing it.
    assert np.allclose(full, fast, rtol=1e-7, atol=1e-9)


def test_restricted_jacobian_handles_a_fitted_background():
    from peak_fit import _make_model, _numeric_jacobian, _parameter_influence

    names = ["amp_0", "pos_0", "sigma", "bg_c0", "bg_c1"]
    model_named = _make_model(
        names, names, {}, n_peaks=1, link_widths=True, enable_left_tail=False,
        background_reference=100.0,
    )
    model = lambda xx, pp: model_named(xx, *pp)
    ...
    def restrict(peak_indices, include_background):
        def evaluate(xx, pp):
            return model_named.evaluate(
                xx, dict(zip(names, pp)), peak_indices, include_background
            )
        return evaluate

    restricted = [
        None if scope is None else restrict(*scope)
        for scope in _parameter_influence(names, 1, True)
    ]
    x = np.arange(60, 140, dtype=float)
    p = np.array([1000.0, 100.0, 4.0, 50.0, 0.02])
    full = _numeric_jacobian(model, x, p)
    fast = _numeric_jacobian(model, x, p, restricted)
    assert np.allclose(full, fast, rtol=1e-5, atol=1e-6)

    # The background columns have exact analytic values -- d/d(bg_c0) is 1
    # everywhere and d/d(bg_c1) is (x - reference) -- so they can be checked
    # against truth rather than against each other. The restricted
    # evaluation is measurably CLOSER, because differencing the full model
    # subtracts a peak amplitude of 1000 from itself to recover a
    # background derivative of order 1 and loses precision doing it.
    exact_c1 = x - 100.0
    assert np.max(np.abs(fast[:, 4] - exact_c1)) < np.max(np.abs(full[:, 4] - exact_c1))
    assert np.allclose(fast[:, 3], 1.0, rtol=0, atol=1e-8)


def test_model_evaluate_subset_sums_only_the_named_peaks():
    from peak_fit import _make_model
    ...
    x = np.arange(60, 140, dtype=float)
    values = {"amp_0": 1000.0, "pos_0": 90.0,
              "amp_1": 700.0, "pos_1": 120.0, "sigma": 4.0}
    both = model_named.evaluate(x, values)
    first = model_named.evaluate(x, values, [0], False)

...
- **`.rpm`**: built natively via `rpmbuild` on the AlmaLinux-8 host. `Requires:` lists the 10 fixed RPM package names above, **plus** `Requires: libcrypt.so.1()(64bit)` as a SONAME capability (not a package name) for the reason documented above — this is the one dependency that can't be written as a single package name valid on both EL8 and EL10. **Known risk to verify during implementation** (not yet tested against this app's actual PyInstaller output): `rpmbuild`'s automatic dependency scanner walks every file under `%files`, including PyInstaller's bundled `_internal/*.so` tree, and can auto-generate spurious extra `Requires:`/`Provides:` from the app's own vendored libraries — a well-known friction point when RPM-packaging PyInstaller onedir bundles. If the first real `rpmbuild` run shows this happening, scope the auto-scanner to just the launcher binary (e.g. `%global __requires_exclude_from ^/opt/spectratools/_internal/.*$`) so only the explicit list above is declared; if it doesn't happen, no action needed.

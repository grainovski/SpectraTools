...
- Both install to `/opt/spectratools/` (standard convention for vendor-distributed apps outside the distro's own repos), with a launcher symlink at `/usr/bin/spectratools` and the existing `packaging/linux/spectratools.desktop` + icon (already built for the AppImage attempt, reused as-is) placed in `/usr/share/applications/` and `/usr/share/pixmaps/` — the latter chosen over a themed `/usr/share/icons/hicolor/...` path since it's a single fixed-size icon with no icon-theme structure to fit into, and (unlike the themed path) needs no `gtk-update-icon-cache`-style post-install scriptlet, which neither package has.
...
- **RHEL/CentOS 7 (or older)** — discovered after this spec's original writing, when the user tested the shipped RPM on a real CentOS 7 machine and hit `GLIBC_2.28' not found`. This is *not* a build-baseline problem the way the original Ubuntu-24.04-vs-CentOS glibc mismatch was — it can't be fixed by building on an even older distro, because PySide6 itself stopped publishing glibc-2.17-compatible wheels starting at version 6.3.0 (the last compatible release was 6.2.4, from 2021); current numpy/scipy have the same floor. This is an upstream constraint from the Python packaging ecosystem, not something this project's build pipeline controls. Confirmed via `pip download`/PyPI wheel-tag inspection before concluding this, not assumed. Discussed with the user directly (`AskUserQuestion`): downgrading the whole dependency stack to ~2021-era versions to support an OS that reached end-of-life in June 2024 was rejected as disproportionate risk for the benefit — RHEL/CentOS/AlmaLinux/Rocky **8 or later** is the real floor, documented in `packaging/linux/INSTALL.md`.

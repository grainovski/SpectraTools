...
**`main_window.py` wiring**: two new `QAction`s, "Add Spectra..." (`Ctrl+A`) and "Subtract Spectra..." (`Ctrl+Shift+A` — same base letter as Add, Shift-modified for the "inverse" operation, both free in the current shortcut inventory), added to the Operations menu after the existing Multiply/Rebin/Normalize entries. Disabled whenever fewer than 2 spectra are loaded (wired into whichever existing mechanism already toggles Multiply/Rebin/Normalize's enabled state as spectra are loaded/closed — the plan will pin down the exact call site). Each opens the new dialog; on a successful result, builds the new `LoadedSpectrum` (next color in the theme's cycle, `active = True`, empty `fits`) and appends it to `self.spectra`, then replots.

**Documentation (`help_content.py`)**: every existing Operations-menu entry is documented in the HowTo page (`build_howto_html()`) — both the shortcut table (`main_window.py:105-111`'s HTML counterpart) and a numbered prose section (currently `<h3>5. Multiply, Rebin, and Normalize</h3>`). This feature is not exempt from that convention. Adds two rows to the shortcut table (`Ctrl+A` / Add Spectra..., `Ctrl+Shift+A` / Subtract Spectra...) and a new `<h3>6. Add and Subtract Spectra</h3>` section immediately after the existing Multiply/Rebin/Normalize section, in the same prose style, explaining: pick two loaded spectra of the same length, the second is scaled by the factor (defaulting to 1) before being added to or subtracted from the first, the result is a new spectrum (originals untouched), and negative counts are possible in a Subtract result and are not an error. Existing sections numbered 6+ shift down by one to make room.
...
- **HowTo page**: existing `tests/test_help_content.py`-style checks extended to confirm the new shortcut-table rows and prose section are present and the page still builds/renders without error — matching the "Help menu: final review" verification already done for every prior feature that touched `help_content.py`.

...
> **ADDENDUM (2026-08-16, v3.1.0 audit): the `Ctrl+C` behavior change described in the next paragraph was implemented as specified, then deliberately REVERTED 11 days later.** `Ctrl+C` hides rather than deletes: `FitModeController.clear()` sets `result.visible = False` (see `fit_mode.py`'s `clear()`), exactly as the "before" state on line 30 above describes. The current behavior is correct and matches the in-app HowTo/Knowledge Database docs, which document `Ctrl+C` as hide and `Ctrl+Shift+C` as delete. **This paragraph is retained as a record of what was decided at the time — do not "restore" deletion by reading it as ground truth.** Flagged by the v3.1.0 audit as a Minor finding precisely because a future reader could mistake it for current intent.

...
- **`tests/test_mtx_io.py`**: small hand-built synthetic `lc`-format files (using the exact byte layout above) for header parsing and basic decode correctness, plus the oracle-verified test vectors described in "Verification strategy" above, generated from the real `gpff.mtx`/`gg.mtx` files via the compiled `libmfile` reference decoder. The two real files themselves (30-31MB each) should **not** be committed to git as fixtures at that size — confirm with the user during planning whether they'd rather track them anyway, keep them untracked-but-present for manual verification (matching this project's existing pattern for large sample files), or something else.

...
**Outcome: 3 were already fixed, 19 fixed here, 2 deliberately not changed (with reasons), 0 blocked.**
...
| M1 | Initial sigma seed runs ~2× TV's scale | Confirmed against source: TV's `FSInitWidth` (`vsFitSetup.c:384`) sets `width = w * FWHM_TO_SIGMA` where `w` is a HALF-width and `FWHM_TO_SIGMA` is 1/2.3548, and TV's `width` really is a sigma (its Gaussian is `exp(-dx*dx*0.5*ss)`, `ss = 1/sig^2`, `vsFit.c:988`/`:1070`). Seed now halved to match. **Not merely parity — measurably better**: over 200 randomised fits the old seed failed to converge 10 times vs 0, mean position error 1.04 → 0.57 ch, worst 19.3 → 10.2 ch, median unchanged. Applied in `_initial_guess`, not inside `_measure_width`, which keeps meaning the measured sigma |
| M2 | Marquardt half-step fallback skips re-damping | TV re-runs the same `CurChangeTry` pass on the reversed half step (`vsCurFit.c:130-134`); this port applied it raw. Not redundant: halving only shrinks a step, but **negating** it can break a constraint the forward step honoured — demonstrated a peak at ch 10.5 landing at 9.5, outside a region starting at 10.0, because `_clamp_trial` bounds sigma and tail parameters but not positions |
...
**M1 and M2** were the two findings originally blocked on the vendored TV source, which was unavailable after the machine move and arrived later in the session. Both were then re-derived from the real C source per this project's standing rule, never from the spec's prose — including confirming that TV's `width` parameter genuinely means a sigma before acting on the factor-of-2, since that reading is what the whole finding rests on.

Full suite after all changes: **820 passed, 2 xfailed, 1 failed** — up from 782 passed (38 tests added). The single failure is `test_load_mtx_decodes_real_fixture_reasonably_fast`, a machine-calibrated wall-clock assertion unrelated to these changes; see that test's own note and project memory. Each behavioural fix was additionally proven to fail before its fix (M10: 17 failures; M17, M18, M12: asserted counts).

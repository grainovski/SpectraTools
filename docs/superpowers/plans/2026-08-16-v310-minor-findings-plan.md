...
**Outcome: 3 were already fixed, 15 fixed here, 4 deliberately not changed (with reasons), 2 blocked.**
...
- **M7 — `draw_committed_fits` is long** (113 → 98 lines after the Important-5 `_peak_component` extraction, which the audit said this pairs with). The audit states it is "not incorrect". A further split of drawing code carries real regression risk with no behavioural payoff and nothing asserting rendered output.

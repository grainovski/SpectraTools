...
<!-- Post-review fix (final holistic review): HowTo said "share one FWHM"
     unconditionally here, while the Knowledge Database page correctly
     says "by default... unless you turn on Independent widths" (fixed
     in commit ce57765). Both pages describe the same fact, so both need
     to agree -- this was only caught once all 4 pages existed together
     and got compared side by side, which no single-task review could
     have seen. -->
...
<!-- Post-review fix (round 2, commit history around this task): a
     ...
     Round-3 re-review then found one more, lower-severity overclaim:
     "every peak shares one FWHM" was stated as absolute, but it's only
     the default -- fit_mode.py has a real "Independent widths" checkbox
     (tooltip: "Fit each peak's width independently instead of sharing
     one FWHM") wired to fit_peaks(link_widths=...), proven by the
     already-passing tests/test_peak_fit.py::
     test_fit_independent_widths_recovers_different_sigmas. tail_fraction/
     tail_beta have no such override (re-confirmed), so only the FWHM
     claim needed qualifying, not the other two. -->
<h2>Multiplets</h2>
<figure>
...
```python
<!-- Post-review fix: this plan originally had `from PySide6.QtGui import
     QKeySequence, QMenu` -- QMenu is actually in PySide6.QtWidgets, not
     QtGui (confirmed: `from PySide6.QtGui import QMenu` raises
     ImportError). tests/test_operations_menu.py's own working _menu_named
     helper already imports them separately; this plan's snippet just
     combined them incorrectly. The implementer caught this because the
     wrong import made Step 2's tests fail with a collection-time
     ImportError instead of the expected AttributeError. -->
...
<!-- Post-review fix: this step originally also called for adding QUrl
     to the QtCore import and QDesktopServices to the QtGui import
     below. The implementer correctly caught that neither is ever
     referenced in main_window.py -- the three new handler methods only
     call open_help_page(...), and open_help_page already wraps
     QUrl/QDesktopServices internally inside help_content.py. Adding
     them here would have been dead/unused imports in the app's most
     central file. Skip that part of this step; main_window.py's
     QtCore/QtGui imports are unchanged by this task -- only the new
     help_content import below is needed. -->
...
- [ ] **Step 4b: Add caching to `build_knowledge_database_html` in `help_content.py`**

<!-- Post-review addition, not in the original plan: a Task 3
     code-quality review measured build_knowledge_database_html() at
     ~340ms per call (4 matplotlib figure renders, no caching) and
     found this acceptable in isolation but recommended caching "ideally
     alongside Task 6" -- i.e. once the function is actually wired to a
     QAction that runs synchronously on the Qt UI thread, where every
     repeat click on "Knowledge Database" would otherwise re-freeze the
     UI for ~340ms reproducing identical output. The function takes no
     arguments and was independently verified deterministic during that
     review, so lru_cache(maxsize=1) is safe with zero invalidation
     logic needed. -->

In `help_content.py`, add `import functools` to the stdlib import block at the top (alongside `import base64` / `import tempfile`), then add `@functools.lru_cache(maxsize=1)` directly above `def build_knowledge_database_html():`. No new test needed -- Task 3's existing tests for that function still exercise it correctly through the cache.
...
Run: `python -m pytest tests/test_help_content.py -v`
Expected: PASS, all tests (confirms the lru_cache addition didn't break Task 3's tests)
...
```bash
git add main_window.py help_content.py tests/test_help_menu.py

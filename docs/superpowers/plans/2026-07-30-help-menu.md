...
def test_knowledge_database_html_contains_parameter_names():
    ...
    for term in [
        ...
        "tail beta", "background slope", "Gross area", "Net area",
    ]:
...
<h2>Gross area vs. net area</h2>
<p>The Fit Results panel reports two areas per peak. <b>Gross area</b>
is the total area under the fitted peak shape, background included.
<b>Net area</b> subtracts the background line's contribution across the
peak, leaving only the counts attributable to the peak itself -- net
area is almost always the number you actually want (for example, when
computing activity or a branching ratio). Both are reported with
propagated uncertainties.</p>

<h2>Integration vs. fitting</h2>
<p><kbd>Ctrl+I</kbd> (Integrate) computes the same gross/background/net
split directly from the data in the marked regions -- centroid, FWHM,
skewness, and area -- without fitting a peak shape at all. It's faster,
doesn't depend on an optimizer converging, and works on peaks too
irregular or blended to fit cleanly. The trade-off is that it can't

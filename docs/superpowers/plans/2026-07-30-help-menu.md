...
<!-- Pre-implementation fix: the original draft of this section claimed
     "the Fit Results panel reports two areas per peak" using the term
     "Gross area" throughout. Verified against fit_mode.py before Task 3
     was dispatched (same lesson as Task 2's post-review fixes, applied
     proactively this time): the results TABLE shows exactly one number
     per peak (its net area, peak.area) -- the second number only
     appears in the row's hover tooltip, alongside a further pair of
     REGION-level (not per-peak) full/net totals. And the tooltip's own
     wording for a committed fit is "full (no bg subtracted)"/"net (bg
     subtracted)" -- it never says "gross" anywhere (fit_mode.py:888-889,
     908-910). "Gross" IS the real, correct term, but only for
     Integration mode's own tooltip (fit_mode.py:242, literally
     `("Gross", "gross"), ("Background", "background"), ("Net", "net")`)
     -- a genuine, if slightly confusing, terminology difference between
     the two features in the app itself, not something to paper over. -->
